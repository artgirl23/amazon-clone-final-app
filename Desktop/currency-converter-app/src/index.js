...
const CurrencyConverter = () => {
  ...
  //useEffect(() => {
    //axios({
     // method: "GET",
     // url:
    //  "https://free.currconv.com/api/v7/convert?q=USD_AUD&compact=ultra&apiKey=552bd0cd82967a2805a2"
   // })
   //   .then((response) => {
 ...
 // }, []);
  ...
  const getRate = (first, second) => {
    axios({
      method: "GET",
      url:
      `https://free.currconv.com/api/v7/convert?q=${first}_${second}&compact=ultra&apiKey=552bd0cd82967a2805a2`
    })
      .then((response) => {
        console.log(response.data);
